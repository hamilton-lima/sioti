...
public class TestJMEDesktop extends SimpleGame {
	...
	protected void simpleInitGame() {
		...
		// move the 'default' keys (debug normals, toggle lighting, etc.) to a
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					// Only access the Swing UI from the Swing event dispatch
					// thread!
	...
	private void createBoxBorder() {
		...
		Box top = new Box("top border", new Vector3f(), halfDesktopWidth
				+ halfBorderSize, halfBorderSize, halfBorderSize);
		...
		Box bottom = new Box("bottom border", new Vector3f(), halfDesktopWidth
				+ halfBorderSize, halfBorderSize, halfBorderSize);
...
//		desktopNode.getLocalRotation().fromAngleNormalAxis(-0.7f,
//				new Vector3f(1, 0, 0));
	...
	protected void createSwingStuff() {
		...
		final JButton buttonToggleMouse = new JButton(
				"toggle system/custom cursor");
		...
		buttonToggleMouse
				.addActionListener(new JMEAction("toggle mouse", input) {
		...
		final JLabel label = new JLabel(
				"click scene to steer view (WASD+Arrows)");
		label.setSize(label.getPreferredSize());
		label.setLocation(display.getWidth() - (int) label.getSize().getWidth()
				- 10, 10);
		...
		JButton fullScreenButton = new JButton(
				"<html><big>toggle fullscreen</big></html>");
		fullScreenButton.setSize(fullScreenButton.getPreferredSize());
		fullScreenButton.setLocation((display.getWidth() - fullScreenButton
				.getWidth()) / 2, display.getHeight() - 40
	...
	private void createRotateButton(JDesktopPane parent, final float direction) {
		...
		button.setLocation(
				(int) ((display.getWidth() - button.getWidth()) / 2 + direction
						* display.getWidth()), display.getHeight() - 40
						- button.getHeight() / 2);
		parent.add(button);
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (desktopNode.getRenderQueueMode() != Renderer.QUEUE_ORTHO) {
					desktopNode.addController(new Controller() {
						...
						Quaternion start = new Quaternion().set(desktopNode
								.getLocalRotation());

						Quaternion finish = new Quaternion().set(
								desktopNode.getLocalRotation()).multLocal(
								new Quaternion().fromAngleNormalAxis(direction,
										new Vector3f(0, 1, 0)));
						...
						public void update(float time) {
							if (timer.getTimeInSeconds() > endTime) {
								...
								desktopNode.getLocalRotation().slerp(
										finish,
										start,
	...
	private void createMoreSwingStuff() {
		...
		JComboBox comboBox = new JComboBox(new Object[] { "Item 1", "Item 2",
				"Item 3", "4", "5", "6", "7", "8", "9" });
	...
	private void createSwingInternalFrame(final JDesktopPane desktopPane,
			final String title, int x, int y) {
		...
		internalFrame.getContentPane().add(
				new JButton("<html><i>test</i> <big>2</big></html>"));
		internalFrame.setVisible(true);
		internalFrame.pack();
	...
	private void switchLookAndFeel(int theme) {
		try {
			...
			// laf.load( TestJMEDesktop.class.getResourceAsStream( "test.xml" ),
			// TestJMEDesktop.class );
	...
	private void create3DStuff() {
		...
		final Box box = new Box("Box", new Vector3f(-5, -5, -5), new Vector3f(
				5, 5, 5));
		...
		ts.setTexture(TextureManager.loadTexture(
				TestJMEDesktop.class.getClassLoader().getResource(
